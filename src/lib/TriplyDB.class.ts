...
export default class TriplyDB {
  ...
  public async write(pipeline: Pipeline, spinner: Ora): Promise<void> {
    const filenames = Array.from(pipeline.stages.keys())
      .map(stageName => pipeline.stages.get(stageName)!.destinationPath)
    ...
    await this.app!.getAccount(this.accountname)
      ...
      .then(async dataset => {
        ...
        throw new Error(`Failed to upload your data to TriplyDB ${this.accountname}/${this.datasetname}: ${(e as Error).message}.`)
      })
  }
}